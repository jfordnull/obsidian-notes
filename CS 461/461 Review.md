...
- Stochastic Beam Search chooses k successors randomly to avoid becoming a more expensive hill-climbing variant
...
A search technique used to find true or approximate solutions to optimization and search problems. Categorized as global search heuristics. A variant of stochastic beam search in which successors are generated by combining two parent states

Evolutionary algorithms that use concepts from biology such as:
- Inheritance
- Mutation
- Selection
- Crossover (also called recombination), happens in Reproduction phase

Genotype: Particular set of genes in the genome
Phenotype: Physical characteristics of the genotype

Basic approach:
- Begin with population of k randomly generated states
- Evaluate each state according to the fitness/objective function
	- Higher values = better states
- Fitness function is used to assess which states will be allowed to reproduce. There are many approaches to culling: 
	- Cull the least-fit half. Uniformly select from survivors
	- Cull least-fit half. Rate selection by fitness estimate
	- Etc.
- Reproduction

Reproduction:
- A dividing line is chosen randomly; Data from the left of that line from parent P, and from the right of that line from parent Q, are used to create offspring. Referred to as *crossover*
- Each parameter also has a small chance of being changed randomly, referred to as *mutation*. This is how we introduce new variation/information to population/system

If maximum fitness holds steady for several generations, we may have found true maximum, but we may also have reached a plateau and should allow algorithm to run for more generations

Poor/sub-optimal results of GA can be a consequence of:
- Too small population
- Cooling too rapidly in simulated annealing
